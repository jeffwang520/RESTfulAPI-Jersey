...
<title>XXXSecurityWeb Portal</title>
...
function getAuthrizationCode(){
	return getQueryString("code");
}
function initSecurityContext(){
	debugger;
	var authCode = getAuthrizationCode();
		
	if(authCode==null){
		setCookie("AuthrizationCode","","1m");
		setCookie("AccessToken","","1m");
		forwardToOAuthServer();
	}else{
		setCookie("AuthrizationCode",authCode,"1m");
...
function getAccessToken(authCode){
	debugger;
	var data  = "";
	data+="client_id="+encodeURIComponent("XXXSecurityWeb");
	data+="&code="+encodeURIComponent(authCode);
	data+="&redirect_uri="+encodeURIComponent("http://localhost:8080/XXXSecurityWeb/");
    ...
    if(this.readyState === 4) {
		...
		processAccessTokenResponse(this.responseText);
     }
    });

    xhr.open("POST", "http://localhost:8888/auth/realms/HSBC/protocol/openid-connect/token");
...
function processAccessTokenResponse(responseText){
	debugger;
	alert(responseText);
	respJson = eval("(" + responseText + ")");
	var accessToken = respJson.access_token;
	var refreshToken = respJson.refresh_token;
	
	setCookie("AccessToken",accessToken,"1m");
	
}
function forwardToOAuthServer(){
	window.location = "http://localhost:8888/auth/realms/HSBC/protocol/openid-connect/auth?client_id=XXXSecurityWeb&response_type=code&redirect_uri=http://localhost:8080/XXXSecurityWeb/";
}
...
<body class="twoColElsLtHdr" onload="initSecurityContext()">
